...
class DQCNNPCAMini(nn.Module):
    ...
    def __init__(self, env, pca_path, batch_sz=128, lr=0.01, gamma=0.99, regularization=0.0001, target_update=0, anneal=False, loss='Huber'):
        super(DQCNNPCAMini, self).__init__()
    ...
    def create_target_network(self):
        self.target_network = [DQCNNPCAMini(env=self.env, batch_sz=self.batch_size,
    ...
    def load_state_dict(self, state_dict):        
        if self.use_target_network:
            super(DQCNNPCAMini, self).load_state_dict(state_dict)
